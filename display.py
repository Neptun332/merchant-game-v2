...
class Display:
    ...
    def draw_chart(
        self, price_history, grid_x=0, grid_y=0, num_cycles=1000, title=None
    ):
        chart_width = (
            self.width - (self.chart_padding * (self.grid_cols + 1))
        ) // self.grid_cols
        chart_height = (
            self.height - (self.chart_padding * (self.grid_rows + 1))
        ) // self.grid_rows
        ...
        pygame.draw.rect(
            self.screen, (240, 240, 240), (chart_x, chart_y, chart_width, chart_height)
        )
        pygame.draw.rect(
            self.screen, (0, 0, 0), (chart_x, chart_y, chart_width, chart_height), 1
        )
        if title:
        ...
        pygame.draw.line(
            ...
            (0, 0, 0),
            (chart_area_x, chart_area_y + chart_area_height),
            (chart_area_x + chart_area_width, chart_area_y + chart_area_height),
            2,
        )
        if len(price_history) > 1:
            ...
            for i in range(4):
                ...
                y_pos = (
                    chart_area_y
                    + chart_area_height
                    - ((y_value - min_price) / price_range * chart_area_height)
                )
                pygame.draw.line(
                    self.screen,
                    grid_color,
            ...
            if len(price_history) > 10:
                ...
                for i in range(0, len(price_history), step):
                    ...
                    pygame.draw.line(
                        ...
                        grid_color,
                        (x_pos, chart_area_y),
                        (x_pos, chart_area_y + chart_area_height),
                        1,
                    )
                    if i % (step * 2) == 0:
                        text = self.font.render(f"{i}", True, (0, 0, 0))
                        self.screen.blit(
                            text, (x_pos - 10, chart_area_y + chart_area_height + 5)
                        )
            for i in range(1, len(price_history)):
                ...
                y2 = (
                    chart_area_y
                    + chart_area_height
                    - ((price_history[i] - min_price) / price_range * chart_area_height)
                )
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.dragging:
                    ...
                    self.map_offset_x = self.adjust_offset_x(
                        self.map_offset_x + dx, map
                    )
                    self.map_offset_y = self.adjust_offset_y(
                        self.map_offset_y + dy, map
                    )
    ...
    def draw_terrain_map(self, map):
        ...
        if (
            ...
            or self.last_cell_size != self.cell_size
                    ...
                    for i in range(y_chunk, chunk_end_y):
                        for j in range(x_chunk, chunk_end_x):
                            ...
                            if self.cell_size > 1:
                                ...
                                chunk_surface.set_at((j - x_chunk, i - y_chunk), color)
        ...
        visible_rect = pygame.Rect(
            -self.map_offset_x, -self.map_offset_y, map_width, map_height
        )
        self.buffer.fill(self.color_map["DEFAULT"])
        self.buffer.blit(
            self.cached_surface, (self.map_offset_x, self.map_offset_y), visible_rect
        )
    ...
    def draw_city_on_terrain_map(self, map):
        ...
        for city_pos in map.city_positions:
            ...
            scaled_castle = pygame.transform.scale(
                self.castle_sprite, (castle_size, castle_size)
            )

            # Draw the scaled sprite on the screen
            self.screen.blit(
                scaled_castle,
                (screen_x - (castle_size / 2), screen_y - (castle_size / 2)),
            )
