...
class GlobalMarket:
    ...
    def __init__(self, cities: list[City], npcs: list[NPC]):
        ...
        self.price_history = {ResourceName.Iron: [self.base_prices[ResourceName.Iron]]}
    ...
    def update_prices(self):
        ...
        for resource_name in self.price_history.keys():
            ...
            self.consumption_history[resource_name].append(
                self.total_consumed[resource_name]
            )
            self.production_history[resource_name].append(
                self.total_produced[resource_name]
            )
            self.amount_history[resource_name].append(self.total_amount[resource_name])
            self.base_prices_history[resource_name].append(
                self.base_prices[resource_name]
            )
    ...
    def get_resource_price(self, resource_name: ResourceName) -> float:
        return self.base_prices[resource_name] * (
            1 + self.get_resource_price_change(resource_name)
        )
    ...
    def get_resource_supply(self, resource_name: ResourceName) -> int:
        return (
            self.get_recent_average_production(resource_name)
            + self.total_amount[resource_name]
        )

    def get_resource_price_change(self, resource_name: ResourceName) -> float:
        return self.price_change_factor * (
            self.get_resource_demand(resource_name)
            / (self.get_resource_supply(resource_name) + 1)
        )
    ...
    def calculate_total_resource_consumed(self):
        ...
        for local_market in self.local_markets:
            for resource_name in local_market.consumed_resources:
                self.total_consumed[resource_name] += local_market.consumed_resources[
                    resource_name
                ]
    ...
    def calculate_total_resource_produced(self):
        ...
        for local_market in self.local_markets:
            for resource_name in local_market.produced_resources:
                self.total_produced[resource_name] += local_market.produced_resources[
                    resource_name
                ]
    ...
    def calculate_total_resource_amount(self):
        ...
        for local_market in self.local_markets:
            for resource_name in local_market.resources:
                self.total_amount[resource_name] += local_market.resources[
                    resource_name
                ].amount
    ...
    def calculate_base_prices(self):
        for resource_name, market_share in self.market_share.items():
            self.base_prices[resource_name] = market_share / max(
                1, self.total_amount[resource_name]
            )
    ...
    def get_recent_average_production(self, resource_name: ResourceName):
        return (
            sum(
                self.production_history[resource_name][
                    -self.number_of_ticks_for_average :
                ]
            )
            / self.number_of_ticks_for_average
        )
    ...
    def get_proportional_value_for_single_unit(self):
        number_of_units_per_resource = [
            self.price_value_modifiers[resource_name] * ammount
            for resource_name, ammount in self.total_amount.items()
        ]

        return self.total_gold / (sum(number_of_units_per_resource) + 1)

    def get_number_of_units_per_resource(
        self, aditional_resource_ammount: dict[ResourceName, int] = defaultdict(int)
    ):
        return {
            resource_name: self.price_value_modifiers[resource_name]
            * (ammount + aditional_resource_ammount[resource_name])
            for resource_name, ammount in self.total_amount.items()
        }
    ...
    def get_market_share(
        ...
        number_of_units_per_resource = self.get_number_of_units_per_resource(
            aditional_resource_ammount
        )
        price_of_single_unit = self.total_gold / (
            max(1, sum(number_of_units_per_resource.values()))
        )
        market_share = {}
        for resource_name in self.total_amount.keys():
            market_share[resource_name] = (
                number_of_units_per_resource[resource_name] * price_of_single_unit
            )
        return market_share
