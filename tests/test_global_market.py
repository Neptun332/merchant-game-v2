        ...
        [
            ...
            City(
                ...
                LocalMarket(
                    global_market,
                    {
                        ResourceName.Iron: Resource(ResourceName.Iron, 200),
...
def test_get_resource_price(setup_global_market):
    ...
    assert price == global_market.base_prices[ResourceName.Iron] * (
        1 + global_market.get_resource_price_change(ResourceName.Iron)
    )
...
def test_calculate_base_prices(setup_global_market):
    ...
    assert global_market.base_prices[ResourceName.Iron] == global_market.market_share[
        ResourceName.Iron
    ] / max(1, global_market.total_amount[ResourceName.Iron])
...
def test_get_recent_average_consumption(setup_global_market):
    ...
    average_consumption = global_market.get_recent_average_consumption(
        ResourceName.Iron
    )
...
def test_estimate_base_resource_price(setup_global_market):
    ...
    )  # Since there are 0 Iron, adding 1 should give the total gold as the price
